...
# Дефинирај го schema за JSON податоците
schema = StructType([
    StructField("Meta Data", MapType(StringType(), StringType())),  # MAP<STRING, STRING>
    StructField("Time Series (5min)", MapType(StringType(), StructType([  # MAP<STRING, STRUCT>
...
# Функција за обработка на секој бач на податоци
def process_batch(df, epoch_id):
    try:
        # Печати ги податоците за дебагирање
        logging.info(f"Обработка на бач {epoch_id}...")
        df.show(truncate=False)

        # Зачувај ги податоците во Cassandra и PostgreSQL (преку Data Storage Service)
        for row in df.collect():
            ...
            # Овде може да се додаде повик до Data Storage Service за зачувување на податоците
...
if __name__ == "__main__":
    # Иницијализирај Spark сесија
    spark = create_spark_session()
    logging.info("Spark сесијата е успешно креирана.")

    # Читај ги податоците од Kafka
    df = spark \
        .readStream \
        .format("kafka") \
    ...
    # Конвертирај ги податоците од JSON
    json_df = df.selectExpr("CAST(value AS STRING) as value")
    parsed_df = json_df.select(from_json(col("value"), schema).alias("data")).select("data.*")

    # Печати го schema за проверка
    parsed_df.printSchema()

    # Експлодирај ги Time Series податоците
    exploded_df = parsed_df.select(
        ...
        "symbol",
        "timestamp",
        col("data.`1. open`").cast(FloatType()).alias("open"),  # Користи backticks за полиња со специјални карактери
    ...
    # Печати ги резултатите во конзолата и обработувај ги бачовите
